...
int main (){
    ...
    while (t--){
        ...
        int count[26] = {0};  // Array to track if a problem has been solved

        int ans = 0;  // Total number of balloons

        for (char c : s){
            int index = c - 'A';  // Map 'A'-'Z' to indices 0-25
            if (count[index] == 0){
                ans += 2;  // First time solving this problem
                count[index] = 1;  // Mark as solved
            }
            else {
                ans += 1;  // Subsequent solves of the same problem
            }
        }

        cout << ans << endl;  // Output the total number of balloons
    }
    return 0;
}